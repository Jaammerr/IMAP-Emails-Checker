...
def get_proxies() -> List[Proxy]:
    try:
        proxies = read_file(
            os.path.join(CONFIG_DATA_PATH, "proxies.txt"), check_empty=True
        )
...
def get_accounts(file_name: str) -> Generator[Account, None, None]:
    accounts = read_file(os.path.join(CONFIG_DATA_PATH, file_name), check_empty=True)
...
def load_config() -> Config:
    try:
        accounts = list(get_accounts("accounts.txt"))
        proxies = get_proxies()
        ...
        config.accounts = validate_domains(
            config.accounts, config.imap_settings
        )
        return config
